...
\begin{itemize}
  ...
  \item この変更により，変数$\chi_t, \nu_t, \eta_t, \zeta_t, N_t, \phi_t, N_e,t, N_n,t$と，パラメータ$\beta^{FI}, \theta, \rho, \omega$が増加
\end{itemize}
...
\end{align}
\begin{itemize}
  \item 予算制約式に金融仲介業者からの資産移転$\Pi_t$を追加
...
\subsection{Farmer}
\begin{equation}
q_t(k_t-k_{t-1})+R_tb_{t-1}+x_t-ck_{t-1}
=
ak_{t-1}+b_t
\end{equation}
...
1+\varphi_t
=
(\beta(1+\varphi_{t+1})+\mu_t)R_t
\end{equation}

\begin{equation}
q_t(1+\varphi_t)+\beta c\varphi_{t+1}
=
\beta(1+\varphi_{t+1})(a+c+q_{t+1})
+\mu_tq_{t+1}
\end{equation}
...
総量規制ショックを\eqref{eq:a}式に入れた
\begin{equation} \label{eq:a}
b_t
=
\phi_tN_t (1-\varepsilon_t)
\end{equation}
...
\section{IRFの簡単な考察}
※MATLABのfsolveで定常状態を数値的に見つけるとき初期値が必要になるが，今のところはモデルの実験段階なので，初期値についてはAIでパラメータの値と$R$の値を与えて概算的に計算させた．
\begin{itemize}
  \item 1期目のみであるが，Farmerの借入金$b$と不動産価格$q$が総量規制ショックによって下落
  \item 2期目以降から戻っているのはFarmerの金利$R$が調整のために下落しているからである（Kiyotaki-Mooreではこれを防ぐために金利を固定化した）
  \item よって，$R$を内生変数ではなく一定にしなければならないが，方程式と内生変数の数の問題がある
  \item 加えて，公定歩合の引き上げでFarmerの金利が上昇したというショックを与えることができた場合，総量規制ショックの影響はさらに増幅される可能性がある
