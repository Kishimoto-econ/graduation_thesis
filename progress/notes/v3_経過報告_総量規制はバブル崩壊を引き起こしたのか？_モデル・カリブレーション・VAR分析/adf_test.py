...
data = pd.read_csv(
    r"C:\Users\Kohsu\OneDrive\Desktop\graduation_thesis\data\data_quarter.csv",
    header=[0],
    encoding="cp932"
)
# print(data)
...
for varia in variables_type:
    ...
    for reg in reg_types:
        result = adfuller(df[varia], regression=reg, autolag='AIC', maxlag=8)
...
for varia in variables_type_diff:
    print(f"\n===== {varia} =====")
    for reg in reg_types:
        result = adfuller(df_diff[varia], regression=reg, autolag='AIC', maxlag=8)
        print(f"regression = '{reg}'")
        print(f"ADF statistic : {result[0]:.4f}")
        print(f"p-value       : {result[1]:.4f}")
        print("-" * 30)
