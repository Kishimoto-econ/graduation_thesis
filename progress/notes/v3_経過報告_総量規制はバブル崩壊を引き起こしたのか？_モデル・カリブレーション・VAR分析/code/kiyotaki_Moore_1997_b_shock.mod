...
q*(1+phi) + beta*c*phi(+1) = beta*(1+phi(+1))*(sita*(a+c)*k^(sita-1)+ q(+1)) + mu*q(+1)*(1-eb);

[name='Budget constraint']
q*(k - k(-1)) + b(-1)/betap + x = (a+c)*k(-1)^sita + b;
...
phi = (beta*c)/(q+beta*c-alpha*beta*((a+c)*k^(alpha-1)+q)-(betap-beta)*q)-1;
mu = (betap-beta)*(beta*c)/(q+beta*c-alpha*beta*((a+c)*k^(alpha-1)+q)-(betap-beta)*q);
x = c*k;
C = x + m*xp;
Y = C;
end;
