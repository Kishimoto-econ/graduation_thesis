...
\begin{frame}{研究課題}
  \begin{itemize}
    ...
    \item しかし，理論的・実証的な研究は数が少なく，学問的な合意が取れているとは言い難い
    \item その理由として，\textcite{U2012}が指摘するように，総量規制が地価や生産量に与える影響を明示的に捉えることが困難であることが挙げられる（数少ない研究に\textcite{S2009}がある）
...
\begin{frame}{モデルの全体像} \label{zentai}
  （続いた）
  \begin{gather}
...
ただし，$\mathbb{E}_t \varepsilon_{t+j}^k = 0, \, for \, j > 0 \notag$かつ$iid$である
  \begin{itemize}
    ...
    \item \eqref{eq:kinkou2}式より，
      ...
      \end{equation}
  \end{itemize}
さらに\eqref{eq:kinkou1}式では，$k_t$に対するショックが，$Y$には$t+1$期から影響する \\
よって，\eqref{eq:kinkou1}式の添字を変更してスライド\ref{image1}の説明通りにし，ショックを追加する
   \begin{equation}
      x_t + m x'_t = (a + c) k_t (1+\varepsilon_t^k) + m G(k'_t)
   \end{equation}
\end{frame}
