...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs > 4)
    {
      ...
      if (T_order_on_input < 2)
        {
          T_order_mx = mxCreateDoubleScalar(2);
  ...
  if (T_order_on_input < 2)
    switch (T_order_on_input)
      ...
      case 1:
        dynamic_g2_tt(y, x, params, steady_state, T);
      }
  plhs[0] = mxCreateDoubleMatrix(38, 1, mxREAL);
  dynamic_g2(y, x, params, steady_state, T, mxGetPr(plhs[0]));
  if (nlhs == 3)
    {
      plhs[1] = T_order_mx;
      plhs[2] = T_mx;
