...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (!(mxIsInt32(prhs[3]) && mxGetNumberOfElements(prhs[3]) == 4))
    mexErrMsgTxt("sparse_rowval must be an int32 array with 4 elements");
  if (!(mxIsInt32(prhs[5]) && mxGetNumberOfElements(prhs[5]) == 3))
    mexErrMsgTxt("sparse_colptr must be an int32 array with 3 elements");
  ...
  mxArray *residual_mx = mxCreateDoubleMatrix(2, 1, mxREAL);
  double *restrict residual = mxGetPr(residual_mx);
  if (nlhs > 2)
    plhs[2] = residual_mx;
  residual[0]=(1+y[5])-(y[2]*(1+y[5])*params[0]+y[2]*y[6]);
  residual[1]=(y[0]*(1+y[5])+y[5]*params[5]*params[0])-((1+y[5])*params[0]*(y[0]+(params[4]+params[5])*(1-x[1]))+y[0]*y[6]);
  if (nlhs > 3)
    {
  plhs[3] = mxCreateSparse(2, 2, 4, mxREAL);
...
g1_v[0]=1-y[2]*params[0];
g1_v[1]=y[0]+params[5]*params[0]-params[0]*(y[0]+(params[4]+params[5])*(1-x[1]));
g1_v[2]=(-y[2]);
g1_v[3]=(-y[0]);
    }
}
