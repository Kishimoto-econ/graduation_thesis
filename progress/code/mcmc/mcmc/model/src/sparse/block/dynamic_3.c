...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (!(mxIsInt32(prhs[4]) && mxGetNumberOfElements(prhs[4]) == 4))
    mexErrMsgTxt("sparse_rowval must be an int32 array with 4 elements");
  if (!(mxIsInt32(prhs[6]) && mxGetNumberOfElements(prhs[6]) == 3))
  ...
  if (nlhs > 2)
    plhs[2] = residual_mx;
  residual[0]=(y[22]*(1+y[27])+y[49]*params[5]*params[0])-(params[0]*(1+y[49])*(y[44]+(params[4]+params[5])*(1-x[1]))+y[44]*y[28]);
  residual[1]=(1+y[27])-(y[46]*params[0]*(1+y[49])+y[24]*y[28]);
  if (nlhs > 3)
    {
