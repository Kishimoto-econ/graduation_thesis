...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (!(mxIsInt32(prhs[4]) && mxGetNumberOfElements(prhs[4]) == 84))
    mexErrMsgTxt("sparse_rowval must be an int32 array with 84 elements");
  if (!(mxIsInt32(prhs[6]) && mxGetNumberOfElements(prhs[6]) == 69))
    mexErrMsgTxt("sparse_colptr must be an int32 array with 69 elements");
      ...
      if (T_order_on_input < 1)
        {
          ...
          T_mx = mxCreateDoubleMatrix(max(4, mxGetM(T_old_mx)), 1, mxREAL);
          memcpy(mxGetPr(T_mx), mxGetPr(T_old_mx), mxGetM(T_old_mx)*sizeof(double));
        }
      else if (mxGetM(T_mx) < 4)
        mexErrMsgTxt("T_mx should have at least 4 elements");
    }
  else
    {
      T_order_mx = mxCreateDoubleScalar(1);
      T_mx = mxCreateDoubleMatrix(4, 1, mxREAL);
      T_order_on_input = -1;
    }
  ...
  plhs[0] = mxCreateSparse(22, 68, 84, mxREAL);
  mwIndex *restrict ir = mxGetIr(plhs[0]), *restrict jc = mxGetJc(plhs[0]);
  for (mwSize i = 0; i < 84; i++)
    *ir++ = *sparse_rowval++ - 1;
  for (mwSize i = 0; i < 69; i++)
    *jc++ = *sparse_colptr++ - 1;
  dynamic_g1(y, x, params, steady_state, T, mxGetPr(plhs[0]));
  if (nlhs == 3)
    {
      plhs[1] = T_order_mx;
      plhs[2] = T_mx;
