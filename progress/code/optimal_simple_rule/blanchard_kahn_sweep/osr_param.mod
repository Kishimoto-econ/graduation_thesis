...
;
    ...
    gamma_N = 1;
    Kbar    = 6.62;
    A       = theta * rho * betaFI * omega;
    B       = theta * rho * (1 - betaFI) + (1 - theta) * betaFI * omega;
    C       = -(theta * (1 - theta) * (1 - betaFI/betap) + (1 - theta) * betaFI);

    // Stady state Values
    eb_ss       = 0;
    phi_ss      = (-B + sqrt(B^2 - 4*A*C)) / (2*A);
    Psi_ss      = 1/phi_ss;
    zeta_ss     = (1 - omega * phi_ss) / theta;
    chi_ss      = zeta_ss;
    R_ss        = 1/betap + (zeta_ss - 1/betap) / phi_ss;
    q_ss        = a * R_ss / (R_ss - 1);
    kp_ss       = ((q_ss * (1 - betap)) / (betap * alpha))^(1 / (alpha - 1));
    k_ss        = Kbar - kp_ss;
    b_ss        = q_ss * k_ss / R_ss;
    N_ss        = b_ss / phi_ss;
    Nn_ss       = omega * b_ss;
    ...
    mu_ss       = (1 + varphi_ss) * (1 - beta * R_ss) / R_ss;
    x_ss        = c * k_ss;
...
model;
    ...
    q * (kp-kp(-1)) * (1-eq) + bp + xp = kp(-1)^alpha * (1-eY) + (1-theta-omega) * b(-1) + bp(-1) / betap;
    ...
    Ne  = (theta * ((R - 1/betap) * phi(-1) + 1/betap) * N(-1))-eN;
